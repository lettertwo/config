# Generated by Powerlevel10k configuration wizard on 2023-03-05 at 11:16 EST.
...
() {
  ...
  ################################[ prompt_char: prompt symbol ]################################
  # Yellow prompt symbol if the last command succeeded.
  ...
  typeset -g POWERLEVEL9K_PROMPT_CHAR_{OK,ERROR}_VIVIS_CONTENT_EXPANSION='󰄼 '
  # Prompt symbol in overwrite vi mode.
  typeset -g POWERLEVEL9K_PROMPT_CHAR_{OK,ERROR}_VIOWR_CONTENT_EXPANSION='󰄾 '
  ...
  typeset -g POWERLEVEL9K_SHORTEN_FOLDER_MARKER="(${(j:|:)last})"
  ...
  function my_git_formatter() {
    ...
    if (( $1 )); then
      ...
      local       meta='%F'   # black foreground
      local      clean='%5F'  # purple foreground
      local   modified='%5F'  # purple foreground
      local  untracked='%3F'  # black foreground
      local conflicted='%9F'  # red foreground
      local    deleted='%1F'  # magenta foreground
      local      count='%4F'  # blue foreground
    else
      # Styling for incomplete and stale Git status.
      local       meta='%F'  # black foreground
      local      clean='%F'  # black foreground
      local   modified='%F'  # black foreground
      local  untracked='%F'  # black foreground
      local conflicted='%F'  # black foreground
      local    deleted='%F'  # black foreground
      local      count='%F'  # black foreground
    fi
    ...
    (( VCS_STATUS_HAS_CONFLICTED )) && res+="${conflicted}${meta}"
    (( VCS_STATUS_HAS_STAGED     )) && res+="${modified}${meta}"
    (( VCS_STATUS_HAS_UNSTAGED   )) && res+=" ${modified}${meta}"
    (( VCS_STATUS_HAS_UNTRACKED  )) && res+="${untracked}${meta}"
    (( VCS_STATUS_NUM_UNSTAGED_DELETED  )) && res+="${deleted}${meta}"

    [[ -n $VCS_STATUS_ACTION ]] && res+=" ${meta}[${conflicted}${VCS_STATUS_ACTION}${meta}]"

    (( VCS_STATUS_COMMITS_AHEAD && !VCS_STATUS_COMMITS_BEHIND )) && res+=" [ahead ${count}${VCS_STATUS_COMMITS_AHEAD}${meta}]"
    (( VCS_STATUS_COMMITS_BEHIND && !VCS_STATUS_COMMITS_AHEAD )) && res+=" [behind ${count}${VCS_STATUS_COMMITS_BEHIND}${meta}]"
    (( VCS_STATUS_COMMITS_AHEAD && VCS_STATUS_COMMITS_BEHIND  )) && res+=" [ahead ${count}${VCS_STATUS_COMMITS_AHEAD}${meta}, behind ${count}${VCS_STATUS_COMMITS_BEHIND}${meta}]"

    (( VCS_STATUS_PUSH_COMMITS_AHEAD && !VCS_STATUS_PUSH_COMMITS_BEHIND )) && res+=" [push ahead ${count}${VCS_STATUS_PUSH_COMMITS_AHEAD}${meta}]"
    (( VCS_STATUS_PUSH_COMMITS_BEHIND && !VCS_STATUS_PUSH_COMMITS_AHEAD )) && res+=" [push behind ${count}${VCS_STATUS_PUSH_COMMITS_BEHIND}${meta}]"
    (( VCS_STATUS_PUSH_COMMITS_AHEAD && VCS_STATUS_PUSH_COMMITS_BEHIND  )) && res+=" [push ahead ${count}${VCS_STATUS_PUSH_COMMITS_AHEAD}${meta}, behind ${count}${VCS_STATUS_PUSH_COMMITS_BEHIND}${meta}]"

    (( VCS_STATUS_STASHES )) && res+=" ${meta}[${count}${VCS_STATUS_STASHES}${meta} stashed]"
  ...
  typeset -g POWERLEVEL9K_CONTEXT_{REMOTE,REMOTE_SUDO}_CONTENT_EXPANSION='%n@${${(%):-%2m}#ip-*.}'
