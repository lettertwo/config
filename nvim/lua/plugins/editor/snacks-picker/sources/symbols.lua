...
local EXCLUDED = {
  default = {},
  -- lua = {
  --   "Package", -- luals uses "Package" for control flow structures.
  -- },
}
...
local function want(item, ctx)
  ...
  local excluded = EXCLUDED[filetype] or EXCLUDED.default
  if type(excluded) == "boolean" or vim.tbl_contains(excluded, item.kind) then
    return false
  end
