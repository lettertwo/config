...
function actions.recall_toggle(picker, item)
...
function actions.recall_move_up(picker, item)
  local recall_util_ok, recall_util = pcall(require, "plugins.recall.util")
  if not recall_util_ok or not recall_util then
    Snacks.notify.warn("recall utility not found", { title = picker.title })
    return
...
function actions.recall_move_down(picker, item)
  local recall_util_ok, recall_util = pcall(require, "plugins.recall.util")
  if not recall_util_ok or not recall_util then
    Snacks.notify.warn("recall utility not found", { title = picker.title })
    return
...
---@type table<string, snacks.picker.format>
local formats = {}

function formats.marked_filename(item, picker)
...
function formats.marked_buffer(item, picker)
  local ret = {} ---@type snacks.picker.Highlight[]
  ...
  ret[#ret + 1] = { " " }
  vim.list_extend(ret, formats.marked_filename(item, picker))
  return ret
end

---@type table<string, snacks.picker.transform>
local transforms = {}

---@param item snacks.picker.finder.Item
---@param ctx snacks.picker.finder.ctx
function transforms.titlepath(item, ctx)
...
---@param item snacks.picker.finder.Item
---@param ctx snacks.picker.finder.ctx
function transforms.unique_marked(item, ctx)
  local path = Snacks.picker.util.path(item) or item.file
  if not path then
    return false
  end
  ...
  return transforms.titlepath(item, ctx)
end

---@type table<string, snacks.picker.finder>
local finders = {}

function finders.recall(_, ctx)
...
function finders.buffers(opts, ctx)
...
local sources = {
  recall = {
    ...
    format = formats.marked_filename,
    transform = transforms.unique_marked,
    finder = finders.recall,
    on_show = select_current_buffer,
  },

  -- Augment buffers source with recall marks and actions
  buffers = {
    win = {
      input = {
        keys = {
    ...
    format = formats.marked_buffer,
    on_show = select_current_buffer,
    transform = transforms.titlepath,
    finder = finders.buffers,
  },

  -- Define a "switch" source that combines recall marks, buffers, recent files, and files
  switch = {
