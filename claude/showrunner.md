...
- **No artifact / open design** → `EnterPlanMode`, then interview me through the decision branches (`/spar`, `/spar-with-docs`). Plan mode names a plan file under the project's `.scratch/plans/` (`plansDirectory` in `settings.json`) and makes it the only file you can edit; write the interview outcome there, including what the executor won't inherit, and let `ExitPlanMode` be the approval gate. Dispatch `implementer` against that path when the design was resolved in this session.
