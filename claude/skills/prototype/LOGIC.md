...
Build it as a **lightweight TUI** — on every tick, clear the screen and re-render the whole frame, so the user always sees one stable view, never an ever-growing scrollback. Each frame shows the current state (pretty-printed, diff-friendly) and the keyboard shortcuts, initialises once, and re-renders in full after every keystroke. The whole frame should fit on one screen.
