---@class Config.TextCase.Method
---@field key string|string[] Key(s) to trigger this method (appended to "g~" for visual/normal, "gr~" for LSP rename)
---@field desc string Description for which-key and user feedback
---@field apply fun(str: string): string Function that takes a string and returns the transformed string
...
---@param str_or_list string|string[]
---@param fn fun(str: string): string
local function apply_per_line(str_or_list, fn)
  local lines = type(str_or_list) == "string" and vim.split(str_or_list, "\n", { plain = true }) or str_or_list --[[ @as string[] ]]
...
---@class Config.TextCase
local M = {}

---@type Config.TextCase.Method
M.to_upper_case = {
  key = { "u", "U" },
  desc = "TO UPPER CASE",
  apply = function(str)
...
---@param method Config.TextCase.Method
---@return fun()
...
---@param done fun(item?: Config.TextCase.Method)
