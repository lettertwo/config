...
local function mapSymbols(severity)
	local severityMap = {
		[vim.diagnostic.severity.ERROR] = {
			symbol = Config.icons.diagnostics.Error or "✖",
			hlGroup = "DiagnosticError",
		},
		[vim.diagnostic.severity.WARN] = {
			symbol = Config.icons.diagnostics.Warn or "⚠",
			hlGroup = "DiagnosticWarn",
		},
		[vim.diagnostic.severity.INFO] = {
			symbol = Config.icons.diagnostics.Info or "ℹ",
			hlGroup = "DiagnosticInfo",
		},
		[vim.diagnostic.severity.HINT] = {
			symbol = Config.icons.diagnostics.Hint or "➤",
...
local function parseDiagnostics(diagnostics)
	...
	for _, diag in ipairs(diagnostics) do
		local filePath = vim.fs
			.normalize(diag.bufnr and vim.api.nvim_buf_get_name(diag.bufnr) or "")
			:gsub("^" .. escapedcwd .. "/", "")
		if filePath ~= "" then
...
function M.setup()
	Config.on("User", { "MiniFilesExplorerOpen", "MiniFilesBufferUpdate" }, function(args)
		updateDiagnostics(args.data and args.data.buf_id or nil)
	end)
